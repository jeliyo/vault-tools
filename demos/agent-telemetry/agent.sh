...
vault write auth/approle/role/role1 bind_secret_id=true token_policies=demopolicy,all-kv,db token_ttl=5m token_max_ttl=10m
...
# Start the agent
vault agent -log-level=trace -config config/agent.hcl > /tmp/agent.log 2>&1 &
sleep 5
